...
def select_some_htmls():
	_dir = common.crawed_uspto_text_dir + '/'
	with open(common.pid_csv) as f:
		pids = []
		for ln in f:
...
def similarity():
	...
	numpy.save(common.similarity + '_pids',numpy.array(pids))
	del pids

	for i, docs in enumerate([abstracts,claims,descriptions]):
		X_tfidf = vectorize(docs,'tfidf')
		# dist = pairwise_distances(X_tfidf)
		# numpy.save(common.similarity + '_' + prefix[i] + '_tfidf',dist,False)
		# del dist
		# X_count = vectorize(docs,'count')
		# dist = pairwise_distances(X_count)
		# numpy.save(common.similarity + '_' + prefix[i] + '_count',dist,False)
		X_svd_tfidf = svd(X_tfidf,100)
		del X_tfidf
		dist = pairwise_distances(X_svd_tfidf)
		del X_svd_tfidf
		numpy.save(common.similarity + '_' + prefix[i] + '_svd_tfidf',dist,False)
		del dist
		# X_svd_count = svd(X_count,100)
		# dist = pairwise_distances(X_svd_count)
		# numpy.save(common.similarity + '_' + prefix[i] + '_svd_count',dist,False)
		X_lda = lda(docs,100)
		dist = pairwise_distances(X_lda)
		del X_lda
		numpy.save(common.similarity + '_' + prefix[i] + '_lda',dist,False)
		del dist
...
def distance_distribution():
	...
	for i in range(len(prefix)):
		# D_tfidf.append(numpy.load(common.similarity + '_' + prefix[i] + '_tfidf.npy'))
		# D_count.append(numpy.load(common.similarity + '_' + prefix[i] + '_count.npy'))
		D_svd_tfidf.append(numpy.load(common.similarity + '_' + prefix[i] + '_svd_tfidf.npy'))
		# D_svd_count.append(numpy.load(common.similarity + '_' + prefix[i] + '_svd_count.npy'))
		D_lda.append(numpy.load(common.similarity + '_' + prefix[i] + '_lda.npy'))
	...
	for dist_matrices in [D_svd_tfidf,D_lda]:
		for matrix in dist_matrices:
			...
			curves.append(curve)
	print len(curves)
	with open(common.sim_distribution,'w') as f:
...
def statistics():
	# pkl = pickle.load(open(common.similarity_pkl))
	# X_lda = pkl['claims'+'lda']
	# print X_lda[100,200]
	# numpy.save(common.similarity + 'lda',numpy.ones((5000,5000)),False)
	# ar = numpy.load(common.similarity + 'lda.npy')
	# print ar[100,100]
	# curves = [1]
	# with open(common.sim_distribution,'w') as f:
	# 	json.dump(curves,f)
