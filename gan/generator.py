...
class HawkesGenerator(object):
	...
	def create_trainable_model(self,sequences, pred_length, weights={}, infinite_dimension=False):
		...
		class HawkesLayer(Layer):
			def __init__(self, sequences_value, pred_length, delta = 1., **kwargs):
				...
				self.spont_initializer = Constant(np.ones([self.nb_sequence,self.nb_type]))
				self.Theta_initializer = Constant(np.ones([self.nb_sequence,self.nb_type]))
				self.W_initializer = Constant(np.array([[0.5 for j in range(self.nb_type)] for i in range(self.nb_sequence)]))
				self.Alpha_initializer = Constant(np.array([[0.5 for j in range(self.nb_type)] for i in range(self.nb_sequence)]))
